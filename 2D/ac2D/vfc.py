...
def create_vfc_kernel_2d(
    size: int, sigma: float = 3.0
...
def apply_vfc_2d(
    edge_map: np.ndarray, kx: np.ndarray, ky: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    """
...
def medialness_2d(
    edge_map: np.ndarray, kernel_size: int = 5, sigma: float = 3.0
) -> np.ndarray:
    """
    ...
    medialness = 1 - (magnitude - np.min(magnitude)) / (
